...
`e2ee-kit`은 두 당사자 간 데이터를 종단 간 암호화하는 데 필요한 핵심 암호
모듈을 제공합니다: X25519 키페어, 인증된 세션, ChaCha20-Poly1305 메시지,
Argon2id 비밀번호 박스. 서버 불필요, 런타임 설정 불필요, 최소 의존성.
...
fn main() -> Result<(), e2ee_kit::Error> {
    ...
    // ... `envelope`를 아무 전송 계층으로 Bob에게 보냅니다 ...
...
version 바이트 / 헤더는 AEAD 추가 인증 데이터(AAD)로 인증됩니다. 랜덤 논스는
메시지마다 OS CSPRNG에서 생성됩니다.
...
- **키 인증.** 상대의 공개키는 반드시 대역 외 채널로 인증해야 합니다
  (직접 대조, 신뢰할 수 있는 디렉터리 등). 이 크레이트는 자신의 공개키로
