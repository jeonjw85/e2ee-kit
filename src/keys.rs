...
fn is_low_order(bytes: &[u8; 32]) -> bool {
    ...
    for point in LOW_ORDER_POINTS.iter() {
        let mut diff = 0u8;
        for i in 0..32 {
            diff |= candidate[i] ^ point[i];
        }
        hit |= ((diff as u16).wrapping_sub(1) >> 8) as u8;
    }
    hit & 1 == 1
}
...
impl PublicKey {
    ...
    ///
    /// Returns [`Error::InvalidPublicKey`] for known Curve25519 low-order
    /// points, including the all-zero encoding and the order-8 subgroup
    /// representatives, so a peer cannot force the X25519 shared secret to a
    /// small set of predictable values. The high bit of the last byte is
    /// masked before the check to match the canonical Montgomery encoding.
...
mod tests {
    ...
    fn low_order_public_keys_are_rejected() {
        for point in super::LOW_ORDER_POINTS.iter() {
            assert!(matches!(
